    ...
    <div className="gamePage bg-[#232528e8] h-[100vh] flex flex-col justify-evenly ">
      <div className="gamePage-top text-center text-white text-3xl ">
        <div className="player-names">
          {gameMetadata?.blackPlayer.name} &nbsp; {gameId !== "random" && "vs"}{" "}
          &nbsp; {gameMetadata?.whitePlayer.name} <br />
        </div>
        {result && <div className="win text-center">{result}</div>}
      </div>

      <div className="gamePage-bottom grid grid-cols-1 lg:grid-cols-12 justify-center items-center">
        <div className="chessboard p-5 col-span-8 flex flex-col gap-3 justify-center items-center">
          {playerColor === "b" && <p className="text-white">You</p>}
          <Chessboard
            moves={moves}
            ...
            socket={socket}
          />
          {playerColor === "w" && <p className="text-white">You</p>}
        </div>

        <div className="h-full grid col-span-4 justify-center items-start lg:items-center">
          {!started && (
            <button
              onClick={() => {
                socket.send(
              ...
              className="p-5 bg-zinc-800 text-white px-12 rounded-full hover:bg-zinc-900 cursor-pointer"
            >
              Start Game
            </button>
          )}
          ...
          {started && gameId === "random" && (
            <div className="text-white text-xl">waiting for oppponent...</div>
          )}
